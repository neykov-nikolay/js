...
const filterProductsByUserId = (products: Product[], userId: number): Product[] => {
  return products.filter((product) => product.userId === userId);
};

const updateUser = (user: BaseUser, products: Product[]): User => {
  return { ...user, products };
};

const addProductsToUsers = async () => {
  const [allUsers, allProducts] = await Promise.all([fetchUsers(), fetchProducts()]);
  // const fetchedUsers = await fetch(USERS_URL);
  // const fetchedProducts = await fetch(PRODUCTS_URL);

  const result = allUsers.reduce<User[]>((users, currentUser) => {
    // const filteredProductsByUserId = allProducts.filter(({ userId }) => userId === currentUser.id);
    const filteredProductsByUserId = filterProductsByUserId(allProducts, currentUser.id);

    const updatedUser = updateUser(currentUser, filteredProductsByUserId);
